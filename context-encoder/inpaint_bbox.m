...
img_part_xl = max(box_x-0.5*box_size,1);
...
imfilled = imresize(imfilled,[centersize_y,centersize_x]);
img_removed = img;
img_removed(img_part_yt+floor(box_size/2):img_part_yb-floor(box_size/2)-1, img_part_xl+floor(box_size/2):img_part_xr-floor(box_size/2)-1, :) = imfilled;
